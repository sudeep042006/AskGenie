...
import { Search, Zap, User, Settings, CreditCard } from 'lucide-react';
    ...
    <nav className="fixed top-0 w-full z-[100] px-6 py-4 flex justify-between items-center border-b border-white/5 backdrop-blur-md bg-black/20">
      ...
      <div className="flex-1 flex justify-center md:justify-start">
        <div className="flex items-center gap-2 group cursor-pointer">
          {/* The Glowing Aura behind the icon */}
          <div className="absolute w-10 h-10 bg-teal-500 rounded-full blur-[15px] opacity-40 group-hover:opacity-80 transition-opacity"></div>
          
          {/* Genie Lamp / Zap Icon */}
          <div className="relative p-2 bg-white/5 rounded-xl border border-white/10 group-hover:border-teal-500/50 transition-all">
            <Zap className="text-teal-400 w-6 h-6" fill="currentColor" />
          </div>
      ...
      <div className="flex items-center gap-4 md:gap-8">
        ...
        <div className="hidden lg:flex items-center bg-black/40 border border-white/10 rounded-full px-4 py-1.5 focus-within:border-teal-500/50 focus-within:shadow-[0_0_15px_rgba(45,212,191,0.1)] transition-all">
          ...
          <input 
            type="text" 
            className="bg-transparent border-none focus:ring-0 text-sm w-48 placeholder:text-gray-600" 
